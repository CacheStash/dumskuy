...
import { getRegistryCheckerUrls } from '../services/fontNameService';
import { KNOWN_FONT_CATALOG, detectLigatures } from '../data/fontNamePresets';
...
const PRESET_LIGATURES = ['none', 'ss', 'tt', 'ff', 'fi', 'fl', 'll', 'oo', 'rr', 'st', 'th', 'ee', 'ch'];
...
export const FontNameFinder: React.FC<FontNameFinderProps> = ({
  ...
  const manualQueryUpper = manualCheckName.trim().toUpperCase();
  const manualIsTaken = manualQueryUpper ? KNOWN_FONT_CATALOG.has(manualQueryUpper) : false;
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">
                Font Name Discovery & Registry Availability Checker
              </h2>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Find memorable font names inspired by islands, geography & maps, optimized for ligatures, and check if already taken on major font marketplaces.
            </p>
          </div>
        ...
        <div className="relative max-w-2xl">
          <div className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Have a font name idea? Type here to check availability across MyFonts, Google, DaFont..."
              className="w-full pl-10 pr-24 py-3 text-sm bg-slate-50 dark:bg-studio-950 border border-slate-200 dark:border-studio-750 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 text-slate-800 dark:text-slate-100"
            />
            {manualCheckName && (
              <button
                onClick={() => setManualCheckName('')}
                className="absolute right-3 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 px-2 py-1"
              >
                Clear
              </button>
            )}
          ...
          {manualQueryUpper && manualUrls && (
            <div className="mt-3 p-4 bg-slate-50 dark:bg-studio-950 rounded-xl border border-slate-200 dark:border-studio-800 space-y-3 animate-in fade-in">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-3">
                  ...
                  {manualIsTaken ? (
                    <span className="px-2.5 py-0.5 text-xs font-semibold rounded-full bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/30 flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3" /> Known Existing Font Name
                    </span>
                  ) : (
                    <span className="px-2.5 py-0.5 text-xs font-semibold rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3" /> No Famous Collision
                    </span>
                  )}
                </div>
              ...
              {manualLigs.length > 0 && (
                <div className="flex items-center gap-1.5 text-xs text-slate-500">
                  <span>Detected Ligature Opportunities:</span>
                  {manualLigs.map(l => (
              ...
              <div className="pt-2 border-t border-slate-200/60 dark:border-studio-850">
                <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2">
                  Verify Availability Across Registries:
                </p>
                <div className="flex flex-wrap gap-2">
                  ...
                  <a
                    href={manualUrls.adobeFonts}
                    target="_blank"
                    rel="noreferrer"
                    className="px-3 py-1.5 rounded-lg bg-white dark:bg-studio-850 border border-slate-200 dark:border-studio-700 text-xs font-medium hover:border-brand-500 text-slate-700 dark:text-slate-200 flex items-center gap-1.5 transition-colors"
                  >
                    <span>Adobe Fonts</span> <ExternalLink className="w-3 h-3 text-slate-400" />
                  </a>
                  <a
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-studio-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-800 dark:text-slate-100">
              Font Name Generator Controls
            </h3>
          </div>
          <span className="text-xs text-slate-400">
            Powered by 100% Free AI & Rich Map Dataset
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              {wordCount === 'single' ? 'Format 1 kata seperti "Santorini", "Passau"' : wordCount === 'double' ? 'Format 2 baris seperti "Capri Riviera", "Sumba Heritage"' : 'Campuran 1 atau 2 kata'}
            </p>
          </div>
          ...
          <div>
            ...
            >
              <option value="all">Semua Lokasi & Peta (Global & Lokal)</option>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              Kebanyakan font legendaris terinspirasi dari nama tempat & pulau di peta.
            </p>
          </div>
          ...
          >
            {isLoading ? (
              <>
                ...
                <span>Searching & Checking Font Registries...</span>
              </>
            ) : (
              <>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {cand.isKnownTaken ? (
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 flex items-center gap-1">
                        <AlertTriangle className="w-2.5 h-2.5" /> Taken in Catalog
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 flex items-center gap-1">
                        <CheckCircle2 className="w-2.5 h-2.5" /> Free in Catalog
                      </span>
                    )}
                  </div>

                  {/* Registry Availability Checker Links */}
                  <div className="pt-2 border-t border-slate-100 dark:border-studio-800/80">
                    <div className="flex items-center justify-between text-[10px] font-semibold text-slate-400 uppercase mb-1.5">
                      <span>Marketplace Availability Checker</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-2 py-1 text-[11px] rounded-md bg-slate-50 dark:bg-studio-800 hover:bg-slate-100 dark:hover:bg-studio-750 text-slate-600 dark:text-slate-300 border border-slate-200/60 dark:border-studio-700 flex items-center gap-1 transition-colors"
                        title="Search name availability on MyFonts.com"
                      >
                        <span>MyFonts</span> <ExternalLink className="w-2.5 h-2.5 opacity-60" />
                      </a>
                      <a
                        href={urls.googleFonts}
                        target="_blank"
                        rel="noreferrer"
                        className="px-2 py-1 text-[11px] rounded-md bg-slate-50 dark:bg-studio-800 hover:bg-slate-100 dark:hover:bg-studio-750 text-slate-600 dark:text-slate-300 border border-slate-200/60 dark:border-studio-700 flex items-center gap-1 transition-colors"
                        title="Search Google Fonts"
                      >
                        <span>Google</span> <ExternalLink className="w-2.5 h-2.5 opacity-60" />
                      </a>
                      <a
                        href={urls.dafont}
                        target="_blank"
                        ...
                        title="Search DaFont"
                      >
                        <span>DaFont</span> <ExternalLink className="w-2.5 h-2.5 opacity-60" />
                      </a>
                      <a
                        href={urls.adobeFonts}
                        target="_blank"
                        rel="noreferrer"
                        className="px-2 py-1 text-[11px] rounded-md bg-slate-50 dark:bg-studio-800 hover:bg-slate-100 dark:hover:bg-studio-750 text-slate-600 dark:text-slate-300 border border-slate-200/60 dark:border-studio-700 flex items-center gap-1 transition-colors"
                        title="Search Adobe Fonts"
                      >
                        <span>Adobe</span> <ExternalLink className="w-2.5 h-2.5 opacity-60" />
                      </a>
                      <a
                        ...
                        rel="noreferrer"
                        className="px-2 py-1 text-[11px] rounded-md bg-slate-50 dark:bg-studio-800 hover:bg-slate-100 dark:hover:bg-studio-750 text-slate-600 dark:text-slate-300 border border-slate-200/60 dark:border-studio-700 flex items-center gap-1 transition-colors"
                        title="Check Trademark on USPTO"
                      >
                        <span>Trademark</span> <ExternalLink className="w-2.5 h-2.5 opacity-60" />
                      </a>
                    </div>
                  </div>
